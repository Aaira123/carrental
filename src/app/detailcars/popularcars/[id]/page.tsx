...
export default async function dynamicpage(props: any) {
  ...
  const popularcars = await client.fetch(query);

  // Find the specific car by ID
  const car = popularcars.find((car: any) => car._id === id);
      ...
      <div className='md:flex md:justify-between gap-2'>
        ...
        <div className="md:w-[50%]">
          <div className="bg-[rgba(84,166,255,100%)]  h-[400px] sm:h-[350px] rounded-xl bg-[url('/BG.png')] bg-cover bg-center relative">
            <div className='p-6'>
              ...
              <Link href={`/billing/popularcars/${car._id}`}><button className='bg-[rgba(53,99,233,100%)] px-10 py-4 mt-5 text-white'>Rent Car</button></Link>
            </div>
            <div className='flex justify-center'>
                <Image src={urlFor(car.image).url()} alt='loading' width={400} height={200}className="w-[200px] sm:w-[230px] lg:w-[300px]"></Image>
            </div>
        </div>
        ...
        <div className='mt-3 md:mt-0 md:w-[50%] p-4 bg-white flex flex-col justify-between'>
          ...
          <div className="flex justify-between items-center my-4 lg:my-0">
            ...
            <Link href={`/billing/popularcars/${car._id}`}><button className='bg-[#3563E9] text-white rounded-md px-6 py-2'>Rent Now</button></Link>
          </div>
